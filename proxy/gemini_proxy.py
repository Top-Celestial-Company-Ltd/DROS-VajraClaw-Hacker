...
def get_concurrency_limit() -> int:
    """動態取得當前授權的併發限制數：試用版為 2，正式 Hacker 版為 5"""
    license_key = os.environ.get("LEMONSQUEEZY_LICENSE_KEY", "")
    if license_key.strip() == "VCLAW-FREE-TRIAL-KEY":
        return 2
    return 5
...
async def chat():
    ...
    async with concurrency_lock:
        if ACTIVE_CONNECTIONS >= limit:
            print(f"\n🚨 [DROS 併發熔斷] 當前連線數 ({ACTIVE_CONNECTIONS}) 已達當前授權方案上限 ({limit})！直接封鎖新請求。")
            return jsonify({
                "error": {
                    "message": f"CONCURRENCY_LIMIT_EXCEEDED: Your current license allows a maximum of {limit} concurrent connections. Please upgrade your tier.",
                    "type": "concurrency_limit_error",
                    "code": 429
                }
            }), 429
        ACTIVE_CONNECTIONS += 1
        print(f"\n⚡ [DROS 併發註冊] 新增連線！當前併發計數: {ACTIVE_CONNECTIONS}/{limit}")

    # 用於追蹤是否將釋放職責移交給非同步串流生成器
    delegated_to_stream = False
...
*Dharma Reasoning OS v7.2 — 金剛治學，菩薩度眾，理事無礙。*"""
        ...
        if is_stream:
            # 移交併發管理職責給非同步生成器
            delegated_to_stream = True
            
            async def generate():
                global ACTIVE_CONNECTIONS
                limit_inner = get_concurrency_limit()
                try:
                    ...
                    async with concurrency_lock:
                        if ACTIVE_CONNECTIONS > 0:
                            ACTIVE_CONNECTIONS -= 1
                            print(f"🧹 [DROS 併發釋放] 串流結束，釋放連線。當前併發計數: {ACTIVE_CONNECTIONS}/{limit_inner}")
            ...
            try:
                ...
                async with concurrency_lock:
                    if ACTIVE_CONNECTIONS > 0:
                        ACTIVE_CONNECTIONS -= 1
                        print(f"🧹 [DROS 併發釋放] 非串流請求完成，釋放連線。當前併發計數: {ACTIVE_CONNECTIONS}/{limit}")

    except Exception as e:
        print(f"[!] 系統錯誤: {e}")
        # 若在移交給串流前就發生錯誤，安全解鎖
        if not delegated_to_stream:
            async with concurrency_lock:
                if ACTIVE_CONNECTIONS > 0:
                    ACTIVE_CONNECTIONS -= 1
                    print(f"🧹 [DROS 異常釋放] 處理請求中斷，釋放連線。當前併發計數: {ACTIVE_CONNECTIONS}/{limit}")
